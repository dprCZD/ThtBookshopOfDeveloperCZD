	...
	<div id="menu">
		<ul>
			...
			<ul class="rl">
				...
				<c:if test="${user.id!=-999&&user!=null}"><a href="/Shoppingcar">购物车</a></c:if>
	...
	<table>
		<tr>
			...
			<td>
				...
				<c:if test="${user.id!=-999&&user.id!=null}">欢迎，程序猿${user.loginName}</c:if>
				</p>
			</td>
			<td width="150px"></td>
		</tr>
						...
						<tbody class="text-center">
							<c:forEach items="${requestScope.books }" var="book">
								<tr>
									...
									<td>${book.author}</td>
									<td>${book.price}</td>
									<td><a href="/Bookinfo?bookid=${book.id}">查看详情</a>
									<c:if test="${(user.id!=-999&&user.id!=null&&user.level=='user')}">
									<input type="button" name="add" value="加入购物车" onclick="window.location.href='/add?bookid='+${book.id}+'&url=java'"></td>
									</c:if>
								</tr>
							</c:forEach>
						</tbody>
